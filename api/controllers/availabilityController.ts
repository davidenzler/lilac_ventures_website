...
const addAvailability= async (req,res) => {
    ...
    if(!date||!time) return res.status(400).json({'message': 'Hi'});
    try{
...
const editAvailability= async (req,res) => {
    ...
    if(!dates||!time) return res.status(400).json({'message': 'Incomplete availability. Please choose date and times','date':dates,'time':time});
    try{
