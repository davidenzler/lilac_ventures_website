...
const Form: React.FC<FormProps> = ({ name, onUploadSuccess, currentUser }) => {
    ...
    return (
        <div style={{ display: "flex", marginBottom: "10px" }}>
            ...
            {showUpload && <PdfUpload text={name} username={currentUser} step="5" onFileUploadSuccess={handleUploadSuccess} />}
        </div>
    );
}
...
const fetchFormNames = async () => {
    try {
      const response = await axios.get('/pdfStepMapping/5');
...
export default function Step5(props: StepProps) {
    ...
    return (
        <div>
            <div>Baby Step 5</div>
            <div>Save for your children’s college fund.</div>
            <br />
            {formNames.map((name) => (
