...
const Form: React.FC<FormProps> = ({ name, onUploadSuccess, currentUser }) => {
    ...
    return (
        <div style={{ display: "flex", marginBottom: "10px" }}>
            ...
            {showUpload && <PdfUpload text={name} username={currentUser} step="6" onFileUploadSuccess={handleUploadSuccess} />}
        </div>
    );
}
...
const fetchFormNames = async () => {
    try {
      const response = await axios.get('/pdfStepMapping/6');
...
export default function Step6(props: StepProps) {
    ...
    return (
        <div>
            <div>Baby Step 6</div>
            <div>Pay off your home early.</div>
            <br />
            {formNames.map((name) => (
