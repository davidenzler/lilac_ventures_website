...
const Form: React.FC<FormProps> = ({ name, onUploadSuccess, currentUser }) => {
    ...
    return (
        <div style={{ display: "flex", marginBottom: "10px" }}>
            ...
            {showUpload && <PdfUpload text={name} username={currentUser} step="4" onFileUploadSuccess={handleUploadSuccess} />}
        </div>
    );
}
...
const fetchFormNames = async () => {
    try {
      const response = await axios.get('/pdfStepMapping/4');
...
export default function Step4(props: StepProps) {
    ...
    return (
        <div>
            <div>Baby Step 4</div>
            <div>Invest 15% of your household income in retirement.</div>
            <br />
            {formNames.map((name) => (
